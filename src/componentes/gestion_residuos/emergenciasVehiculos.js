...
import AgregarEmergencia from './crearEmergenciaVehiculos';
import ModificarEmergencia from './modificarEmergenciaVehiculos';
import VerEmergencia from './verEmergenciaVehiculos';
...
export default class EmergenciasResiduos extends Component {
    ...
    obtenerEmergencias = async (e) => {
        var componente = this;
        const res = Axios.post('/api/gestion-residuos/emergencias/vehiculos/', { pagina: this.state.pagina }, { headers: authHeader() }) //se envia peticion axios con el token sesion guardado en local storage como cabecera
            .then(function (res) {
                ...
                return (
                    <div className='custom-confirm '>
                        ...
                        <button className="boton-generico btazul"
                            onClick={() => {
                                const res = Axios.post('/api/gestion-residuos/emergencias/vehiculos/eliminar',
                                    { id: id, },
    ...
    render() {
        // let rutFormateado = funciones.getRutFormateado(emergencia.datosConductor.rut, emergencia.datosConductor.dv);
        let componente = this;
        let emergencias;
        if (this.state.emergencias && this.state.emergencias.length !== 0) {
            emergencias = this.state.emergencias.map((emergencia, index) =>
                <tr className="elemento">
                    ...
                    <td>{emergencia.patente}</td>
                    <td>
                        <span>{emergencia.datosConductor.nombre}  {emergencia.datosConductor.apellido}</span>
                        <span>Rut {componente.formatearRutListado(emergencia.datosConductor.rut, emergencia.datosConductor.dv)}</span>
                    </td>
                ...
                <div className="principal menu-lista-dashboard">
                    <div>
                        <div className="prehead">
                            <h3 className="tituloencuesta verde">Emergencias Vehiculos</h3>
