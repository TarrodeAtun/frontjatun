...
export default class EquipoTrabajador extends Component {
    ...
    obtenerRegistrosDesempeno = async () => {
        var componente = this;
        await Axios.post('/api/users/worker/desempeno/', { rut: autenticacion.currentUserValue.data.usuariobd.rut, fecha: this.state.fecha }, { headers: authHeader() }) //se envia peticion axios con el token sesion guardado en local storage como cabecera
    ...
    render() {
        ...
        return (
            <div className="principal" id="component-perfil">
                <div>
                    <h2 className="celeste"><Link to={`/perfil/ficha-personal`}> <Bcelesterev /> </Link> Ficha / <strong>Indicadores de Desempeño</strong></h2>
