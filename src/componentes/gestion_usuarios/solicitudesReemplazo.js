...
export default class ListarTrabajadores extends Component {
    ...
    numPaginasTurnos = async () => {
        ...
        const res = Axios.post('/api/users/worker/turnos/solicitudes/paginas/', { dato: "nada" }, { headers: authHeader() }) //se envia peticion axios con el token sesion guardado en local storage como cabecera
    ...
    listadoSolicitudesReemplazo = async () => {
        var componente = this;
        var rut = autenticacion.currentUserValue.data.usuariobd.rut;
        const res = Axios.post('/api/users/worker/turnos/listado-solicitudes', { pagina: this.state.pagina }, { headers: authHeader() }) //se envia peticion axios con el token sesion guardado en local storage como cabecera
            .then(function (res) {   //si la peticion es satisfactoria entonces
        ...
        if (this.state.registros.length > 0) {
            for (let registro of this.state.registros) {
                ...
                registro.reemplazos.map((elem, ind) => {
                    // if (e.rut === && rutReemplazo === e.reemplazado) {
                    ...
                    registros.push(<tr>
                        ...
                        <td>
                            {registro.datosTrabajador[ind].nombre}  {registro.datosTrabajador[ind].apellido} <br></br>  {registro.datosTrabajador[ind].rut}
                        </td>
                        <td>
            ...
            // registros = this.state.registros.map((elem, index) => { //por cada turno
            //     let rutReemplazo = elem.datosReemplazo[0].rut;
            //     let estado;
            //     elem.reemplazos.find(function (e) {
            //         if (e.rut === autenticacion.currentUserValue.data.usuariobd.rut && rutReemplazo === e.reemplazado) {
            //             if (e.estado === 0) estado = <span className="solicitudesopciones"><button data-rut={ } data-turno={ } title="Aprobar"><Check /></button><button data-rut={ } data-turno={ } title="Rechazar"><Rech /></button></span>;
            //             if (e.estado === 1) estado = <Check />
            //             if (e.estado === 2) estado = <Rech />
            //         }
            //     });
            ...
            <div className="principal" id="component-listar-trabajadores">
                ...
                <div className="listado listado-simple">
                    <div className="encabezado">
                        <h3 className="amarillo inline-block">Solicitudes por trabajador</h3>
